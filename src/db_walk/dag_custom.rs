...
impl DAG {
    ...
    pub fn add_node(&mut self, node: Node) {
        self.nodes.entry(node.data.clone()).or_insert(node);
    }
    ...
    pub fn traverse_graph_from_start(&self, start_data: &str) -> Vec<Node> {
        ...
        // Private recursive DFS function
        self.dfs(start_data, &mut visited, &mut result);

        // Ensure all nodes are visited, even those not connected to the start node
    ...
    pub fn find_node_by_data(&self, target_data: &str) -> Option<Node> {
        let mut visited = std::collections::HashSet::new();
        let mut result = None;

        for (node_data, _) in self.nodes.iter() {
            if visited.contains(node_data) {
                continue;
            }

            if self.dfs_find(&node_data, target_data, &mut visited) {
                result = self.nodes.get(target_data).cloned();
                break;
            }
        }
    ...
    // Specialized DFS method to find a node and stop once found.
    fn dfs_find(
        &self,
        node_data: &str,
        target_data: &str,
        ...
        if visited.insert(node_data.to_string()) {
            if node_data == target_data {
                return true;
            }

            if let Some(node) = self.nodes.get(node_data) {
                for pred in self.find_predecessors(node_data) {
                    if self.dfs_find(&pred.data, target_data, visited) {
                        return true;
                    }
                }
            }
